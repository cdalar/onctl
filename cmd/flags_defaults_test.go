...
func TestCreateFlagsBindToViper(t *testing.T) {
	...
	cases := []struct{ flag, key, def, override string }{
		...
		// GCP (replaces gcp.yaml; gcp.project has no static default, see
		// TestGCloudDefaultProject in internal/providergcp instead).
	...
	// hetzner.vm.type=cpx21 and gcp.type=n1-standard-1): once a flag is
	// marked Changed -- even by setting it back to its original value --
