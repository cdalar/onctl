...
func TestCreateFlagsBindToViper(t *testing.T) {
	...
	cases := []struct{ flag, key, def, override string }{
		...
		// Azure (replaces azure.yaml; azure.subscriptionId/azure.resourceGroup
		// have no static default, see TestResolveAzureIdentifiers in
		// root_test.go instead).
	...
	// hetzner.vm.type=cpx21 and azure.vm.type=Standard_D4s_v3): once a flag
	// is marked Changed -- even by setting it back to its original value --
	// viper's flag layer outranks SetDefault for every key bound to it, so
